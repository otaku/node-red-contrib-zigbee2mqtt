...
module.exports = function (RED: NodeAPI) {
    ...
    RED.httpAdmin.get(NODE_PATH + 'getDevices', function (req, res) {
        ...
        const controller = RED.nodes.getNode(config.controllerID as string);
        const forceRefresh = config.forceRefresh ? ['1', 'yes', 'true'].includes((config.forceRefresh as string).toLowerCase()) : false;
